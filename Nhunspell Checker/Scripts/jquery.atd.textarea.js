...
AtDCore.prototype.findSuggestion = function (element) {
    ...
    if (this.getAttrib(element, 'pre') == undefined) {
        document.getElementById("divMessage").innerHTML = element.innerHTML;
...
AtD.checkCrossAJAX = function (container_id, callback_f) {
    ...
    debugger;
    //Modified
    //After AtD plugin prepares textarea, flow comes here. In the original plugin, a get method was written in csshttprequest.js file which inturn would call the service from their php server to process the text. I modified this call so it goes to our application only. We can remove the  csshttprequest.js file now.
    $.ajax({
        contentType: 'application/json',
        type: 'POST',
        data: JSON.stringify({ 'synopsis': text, 'language': AtD.rpc_css_lang }),
        url: '/Home/CheckSpelling',
        success: function (response) {
            debugger;
            //Modified
            //The xml data that comes here matches the format AtD server sends. This ensures that the UI when corrections need to be done remains the way it was from the begining and we need not spend time coding a way to display the corrections in UI.
            var xml;
...
AtD.checkTextArea = function (id, linkId, after) {
    if (AtD.api_key == undefined || AtD.rpc == undefined) {
        document.getElementById("divMessage").innerHTML = "You need to set AtD.api_key and AtD.rpc to use AtD.checkTextArea()";
        $("#divMessage").dialog("open");
    } else
    ...
    if (options['link'].html() != options['before']) {
        ...
        commChannel(id, {
            ...
            success: function (errorCount) {
                if (errorCount == 0) {
                    document.getElementById("divMessage").innerHTML = AtD.getLang('message_no_errors_found', "No writing errors were found");
                    $("#divMessage").dialog("open");
                }
                AtD.restoreTextArea(id);
            },
            error: function (reason) {
                options['link'].unbind('click', disableClick);
                if (reason == undefined) {
                    document.getElementById("divMessage").innerHTML = AtD.getLang('message_server_error_short', "There was an error communicating with the spell checking service.");
                    $("#divMessage").dialog("open");
                } else {
                    document.getElementById("divMessage").innerHTML = AtD.getLang('message_server_error_short', "There was an error communicating with the spell checking service.") + "<br><br>" + reason;
                    $("#divMessage").dialog("open");
                }
                AtD.restoreTextArea(id);
            },
